...
/* Recover a callroutine ROM address whose bank byte reads $00 -- the same
 * class of bug fixed in dispatch_tick_callback (callroutine_screen.c): a
 * confirmed-live case where the bank byte of a 24-bit script-encoded
 * address gets silently dropped, and the switch's default case would
 * otherwise just log a warning and silently no-op the whole callroutine
 * call forever after. Checked empirically before adding this: all 230
 * known 24-bit callroutine addresses below have DISTINCT low 16 bits, even
 * spanning 6 different ROM banks (C0-C4, EF), so a bank-0 address whose low
 * word matches exactly one of them is unambiguous -- never a case of
 * silently misrouting a real, different address in another bank. Anything
 * that doesn't match a known low word falls through to the default case
 * exactly as before (logged, harmless no-op). */
static const uint32_t known_callroutine_addrs[] = {
    ROM_ADDR_UNDRAW_FLYOVER_TEXT,
    ROM_ADDR_UPDATE_MINI_GHOST_POSITION,
    ROM_ADDR_UPDATE_BATTLE_SCREEN_EFFECTS,
    ROM_ADDR_SPAWN_ENTITY,
    ROM_ADDR_DECOMPRESS_TITLE_DATA,
    ROM_ADDR_LOAD_TITLE_PALETTE,
    ROM_ADDR_CYCLE_ENTITY_PALETTE,
    ROM_ADDR_SET_STATE_PAUSED,
    ROM_ADDR_SHOW_ENTITY_SPRITE,
    ROM_ADDR_UPDATE_PALETTE_ANIM,
    ROM_ADDR_SET_STATE_RUNNING,
    ROM_ADDR_FINALIZE_PALETTE_FADE,
    ROM_ADDR_LOAD_FILE_SELECT_PALETTES,
    ROM_ADDR_FILL_PALETTES_WHITE,
    ROM_ADDR_FILL_PALETTES_BLACK,
    ROM_ADDR_LOAD_GAS_STATION_PALETTE,
    ROM_ADDR_LOAD_GAS_STATION_FLASH_PALETTE,
    ROM_ADDR_SET_ENTITY_DIRECTION_AND_FRAME,
    ROM_ADDR_DEALLOCATE_ENTITY_SPRITE,
    ROM_ADDR_IS_X_LESS_THAN_ENTITY,
    ROM_ADDR_IS_Y_LESS_THAN_ENTITY,
    ROM_ADDR_MOVEMENT_CMD_CLEAR_LOOP_COUNTER,
    ROM_ADDR_MOVEMENT_CMD_LOOP,
    ROM_ADDR_RENDER_ENTITY_SPRITE_ENTRY3,
    ROM_ADDR_RENDER_ENTITY_SPRITE_ME1,
    ROM_ADDR_RENDER_ENTITY_SPRITE_ME2,
    ROM_ADDR_RESET_ENTITY_ANIMATION,
    ROM_ADDR_SET_ENTITY_MOVEMENT_SPEED,
    ROM_ADDR_SET_ENTITY_MOVEMENT_SPEED_ENTRY2,
    ROM_ADDR_GET_ENTITY_MOVEMENT_SPEED,
    ROM_ADDR_SET_DIRECTION8,
    ROM_ADDR_SET_DIRECTION,
    ROM_ADDR_SET_SURFACE_FLAGS_CR,
    ROM_ADDR_SET_ENTITY_VELOCITY_FROM_DIR,
    ROM_ADDR_MOVE_ENTITY_DISTANCE,
    ROM_ADDR_HALVE_ENTITY_DELTA_Y,
    ROM_ADDR_LOAD_CURRENT_MAP_BLOCK_EVENTS,
    ROM_ADDR_REVERSE_DIRECTION_8,
    ROM_ADDR_QUANTIZE_ENTITY_DIRECTION,
    ROM_ADDR_CALCULATE_DIRECTION_TO_TARGET,
    ROM_ADDR_DISABLE_ENTITY_COLLISION2,
    ROM_ADDR_MOVEMENT_DISPLAY_TEXT,
    ROM_ADDR_MOVEMENT_CMD_PLAY_SOUND,
    ROM_ADDR_MOVEMENT_CMD_COPY_SPRITE_POS,
    ROM_ADDR_MOVEMENT_CMD_COPY_LEADER_POS,
    ROM_ADDR_MOVEMENT_STORE_OFFSET_POSITION,
    ROM_ADDR_MOVEMENT_CMD_GET_EVENT_FLAG,
    ROM_ADDR_MOVEMENT_CMD_SET_EVENT_FLAG,
    ROM_ADDR_MOVEMENT_QUEUE_INTERACTION,
    ROM_ADDR_MOVEMENT_CMD_STORE_NPC_POS,
    ROM_ADDR_MOVEMENT_CMD_STORE_SPRITE_POS,
    ROM_ADDR_MOVEMENT_CMD_FACE_TOWARD_NPC,
    ROM_ADDR_MOVEMENT_CMD_FACE_TOWARD_SPRITE,
    ROM_ADDR_MOVEMENT_SET_BOUNDING_BOX,
    ROM_ADDR_MOVEMENT_SET_POS_FROM_SCREEN,
    ROM_ADDR_MOVEMENT_CMD_SET_NPC_ID,
    ROM_ADDR_MOVEMENT_CMD_ANIMATE_PAL_FADE,
    ROM_ADDR_MOVEMENT_CMD_SETUP_SPOTLIGHT,
    ROM_ADDR_MOVEMENT_CMD_APPLY_COLOR_MATH,
    ROM_ADDR_MOVEMENT_CMD_PRINT_CAST_NAME,
    ROM_ADDR_MOVEMENT_CMD_PRINT_CAST_VAR0,
    ROM_ADDR_MOVEMENT_CMD_PRINT_CAST_PARTY,
    ROM_ADDR_MOVE_TOWARD_NO_SPRITE_CB,
    ROM_ADDR_MOVE_TOWARD_REVERSED_CB,
    ROM_ADDR_UPDATE_DIR_VELOCITY_CB,
    ROM_ADDR_ACTIONSCRIPT_PREPARE_ENTITY,
    ROM_ADDR_ACTIONSCRIPT_PREPARE_AT_LEADER,
    ROM_ADDR_ACTIONSCRIPT_PREPARE_AT_SELF,
    ROM_ADDR_ACTIONSCRIPT_FADE_OUT,
    ROM_ADDR_ACTIONSCRIPT_FADE_IN,
    ROM_ADDR_ACTIONSCRIPT_GET_PARTY_MEMBER_POS,
    ROM_ADDR_PREPARE_ENTITY_AT_TELEPORT_DEST,
    ROM_ADDR_DISABLE_OBJ_HDMA,
    ROM_ADDR_INIT_WINDOW_REGISTERS_CR,
    ROM_ADDR_DECOMP_ITOI_PRODUCTION,
    ROM_ADDR_DECOMP_NINTENDO_PRESENTATION,
    ROM_ADDR_PLAY_FLYOVER_SCRIPT,
    ROM_ADDR_CHOOSE_RANDOM,
    ROM_ADDR_TEST_PLAYER_IN_AREA,
    ROM_ADDR_MAKE_PARTY_LOOK_AT_ENTITY,
    ROM_ADDR_MOVEMENT_CMD_CALC_TRAVEL_FRAMES,
    ROM_ADDR_MOVEMENT_CMD_RETURN_2,
    ROM_ADDR_IS_ENTITY_NEAR_LEADER,
    ROM_ADDR_CHECK_ENTITY_OBSTACLES,
    ROM_ADDR_CHECK_ENEMY_MOVEMENT_OBSTACLES,
    ROM_ADDR_CHECK_NPC_PLAYER_OBSTACLES,
    ROM_ADDR_CHECK_PROSPECTIVE_NPC_COLLISION,
    ROM_ADDR_FADE_OUT_WITH_MOSAIC,
    ROM_ADDR_SPAWN_ENTITY_RELATIVE,
    ROM_ADDR_SET_ENTITY_DIRECTION_VELOCITY,
    ROM_ADDR_UPDATE_ENTITY_ANIMATION,
    ROM_ADDR_CLEAR_CURRENT_ENTITY_COLLISION,
    ROM_ADDR_INITIALIZE_PARTY_MEMBER_ENTITY,
    ROM_ADDR_UPDATE_FOLLOWER_VISUALS,
    ROM_ADDR_SRAM_CHECK_ROUTINE_CHECKSUM,
    ROM_ADDR_INFLICT_SUNSTROKE_CHECK,
    ROM_ADDR_UPDATE_ENTITY_SURFACE_FLAGS,
    ROM_ADDR_SET_NPC_DIR_FROM_FLAG,
    ROM_ADDR_IS_ENTITY_COLLISION_ENABLED,
    ROM_ADDR_CHECK_ENTITY_CAN_PURSUE,
    ROM_ADDR_CHECK_ENTITY_CAN_PURSUE_SHORT,
    ROM_ADDR_CALCULATE_DIRECTION_TO_LEADER,
    ROM_ADDR_STORE_LEADER_POS_TO_ENTITY_VARS,
    ROM_ADDR_CALCULATE_SLEEP_FRAMES,
    ROM_ADDR_SET_ENTITY_SLEEP_FRAMES,
    ROM_ADDR_GET_NPC_DEFAULT_DIRECTION,
    ROM_ADDR_SET_ENTITY_DIRECTION_CR,
    ROM_ADDR_INIT_PARTY_POSITION_BUFFER,
    ROM_ADDR_CREATE_ENTITY_BG3,
    ROM_ADDR_CALCULATE_SLEEP_FROM_VARS,
    ROM_ADDR_CHECK_ENTITY_ENEMY_COLLISION,
    ROM_ADDR_GET_OVERWORLD_STATUS,
    ROM_ADDR_CHECK_PROSPECTIVE_ENTITY_COLLISION,
    ROM_ADDR_CHECK_NPC_PATROL_BOUNDARY,
    ROM_ADDR_ENABLE_ALL_ENTITIES,
    ROM_ADDR_INSTANT_WIN_PP_RECOVERY,
    ROM_ADDR_SETUP_ENTITY_COLOR_MATH,
    ROM_ADDR_GET_ENTITY_DIRECTION,
    ROM_ADDR_RAND_HIGH_BYTE,
    ROM_ADDR_MOVEMENT_CMD_SET_DIR_VELOCITY,
    ROM_ADDR_GET_ENTITY_OBSTACLE_FLAGS,
    ROM_ADDR_GET_ENTITY_PATHFINDING_STATE,
    ROM_ADDR_GET_DIRECTION_ROTATED_CLOCKWISE,
    ROM_ADDR_GET_PAD_PRESS,
    ROM_ADDR_GET_PAD_STATE,
    ROM_ADDR_IS_BELOW_LEADER_Y,
    ROM_ADDR_GET_LEADER_DIRECTION_OFFSET,
    ROM_ADDR_GET_CARDINAL_DIRECTION,
    ROM_ADDR_GET_PARTY_COUNT,
    ROM_ADDR_IS_PLAYER_BUSY,
    ROM_ADDR_NULL_CALLROUTINE,
    ROM_ADDR_FORCE_RENDER_ENTITY_SPRITE,
    ROM_ADDR_MOVE_TOWARD_TARGET_CB,
    ROM_ADDR_CHECK_ENTITY_COLLISION_AHEAD,
    ROM_ADDR_SET_DEFAULT_TM,
    ROM_ADDR_RESTORE_ENTITY_CALLBACK_FLAGS,
    ROM_ADDR_SAVE_ENTITY_POSITION,
    ROM_ADDR_RESTORE_ENTITY_POSITION,
    ROM_ADDR_LOAD_INITIAL_MAP_DATA_FAR,
    ROM_ADDR_GET_RANDOM_NPC_DELAY,
    ROM_ADDR_GET_RANDOM_SCREEN_DELAY,
    ROM_ADDR_INIT_OVAL_WINDOW_FAR,
    ROM_ADDR_LOAD_TEXT_LAYER_TILEMAP,
    ROM_ADDR_DISABLE_OTHER_ENTITY_CALLBACKS,
    ROM_ADDR_DISABLE_OTHER_ENTITY_UPDATES,
    ROM_ADDR_DISABLE_ALL_ENTITIES,
    ROM_ADDR_IS_PSI_ANIMATION_ACTIVE,
    ROM_ADDR_UPDATE_DIR_VELOCITY_REVERSED_CB,
    ROM_ADDR_SCALE_DIRECTION_DISTANCE,
    ROM_ADDR_SETUP_PSI_TELEPORT_DEPARTURE,
    ROM_ADDR_STEER_ENTITY_TOWARD_DIRECTION,
    ROM_ADDR_DISABLE_CURRENT_ENTITY_COLLISION,
    ROM_ADDR_STORE_ENTITY_POSITION_TO_VARS,
    ROM_ADDR_DISABLE_HDMA_CHANNEL4_ALT,
    ROM_ADDR_DISABLE_HDMA_CHANNEL4,
    ROM_ADDR_DISABLE_HDMA_CHANNEL5,
    ROM_ADDR_GET_DIRECTION_FROM_PLAYER_TO_ENTITY,
    ROM_ADDR_CHECK_ENEMY_SHOULD_FLEE,
    ROM_ADDR_CHOOSE_ENTITY_DIRECTION_TO_PLAYER,
    ROM_ADDR_RESTORE_ENTITY_POSITION_FROM_VARS,
    ROM_ADDR_SET_ENTITY_RANDOM_SCREEN_X,
    ROM_ADDR_IS_BATTLE_PENDING,
    ROM_ADDR_CLEAR_ENTITY_COLLISION2,
    ROM_ADDR_RESTORE_MAP_PALETTE,
    ROM_ADDR_SETUP_FULLSCREEN_WINDOW_CLIPPING,
    ROM_ADDR_SETUP_DARK_WINDOW_EFFECT,
    ROM_ADDR_SETUP_WHITE_WINDOW_EFFECT,
    ROM_ADDR_SETUP_DUAL_DARK_WINDOW_EFFECT,
    ROM_ADDR_QUANTIZE_DIRECTION,
    ROM_ADDR_ENTITY_COORDS_RELATIVE_TO_BG3,
    ROM_ADDR_REFLECT_ENTITY_Y_AT_TARGET,
    ROM_ADDR_IS_ENTITY_IN_RANGE_OF_LEADER,
    ROM_ADDR_IS_ENTITY_STILL_ON_CAST_SCREEN,
    ROM_ADDR_RESTORE_OVERWORLD_STATE,
    ROM_ADDR_ENABLE_TESSIE_LEAVES_ENTITIES,
    ROM_ADDR_CHECK_SECTOR_USES_MINISPRITES,
    ROM_ADDR_DISABLE_PARTY_MOVEMENT_AND_HIDE,
    ROM_ADDR_ENABLE_PARTY_MOVEMENT_AND_SHOW,
    ROM_ADDR_RAND,
    ROM_ADDR_QUEUE_NPC_TEXT_INTERACTION,
    ROM_ADDR_INIT_OVAL_WINDOW,
    ROM_ADDR_CLOSE_OVAL_WINDOW,
    ROM_ADDR_CHECK_CAST_SCROLL_THRESHOLD,
    ROM_ADDR_SET_CAST_SCROLL_THRESHOLD,
    ROM_ADDR_HANDLE_CAST_SCROLLING,
    ROM_ADDR_UPLOAD_SPECIAL_CAST_PALETTE,
    ROM_ADDR_GET_DELIVERY_ATTEMPTS,
    ROM_ADDR_RESET_DELIVERY_ATTEMPTS,
    ROM_ADDR_CHECK_DELIVERY_ATTEMPT_LIMIT,
    ROM_ADDR_GET_TIMED_DELIVERY_TIME,
    ROM_ADDR_SET_DELIVERY_TIMER,
    ROM_ADDR_DECREMENT_DELIVERY_TIMER,
    ROM_ADDR_QUEUE_DELIVERY_SUCCESS,
    ROM_ADDR_QUEUE_DELIVERY_FAILURE,
    ROM_ADDR_GET_TIMED_DELIVERY_ENTER_SPEED,
    ROM_ADDR_GET_TIMED_DELIVERY_EXIT_SPEED,
    ROM_ADDR_INIT_DELIVERY_SEQUENCE,
    ROM_ADDR_ACTIONSCRIPT_FADE_OUT_WITH_MOSAIC,
    ROM_ADDR_ACTIONSCRIPT_PREPARE_AT_TELEPORT,
    ROM_ADDR_RENDER_ENTITY_SPRITE_ME3,
    ROM_ADDR_ADVANCE_ENTITY_PATH_POINT,
    ROM_ADDR_ADVANCE_ENTITY_TOWARD_LEADER,
    ROM_ADDR_HANDLE_ENEMY_CONTACT,
    ROM_ADDR_SETUP_DELIVERY_PATH_FROM_ENTITY,
    ROM_ADDR_SETUP_DELIVERY_PATH_REVERSE,
    ROM_ADDR_SET_PHOTOGRAPHER_POSITION,
    ROM_ADDR_CLEAR_SPRITEMAP_BUFFER,
    ROM_ADDR_APPLY_ENTITY_PALETTE_BRIGHTNESS,
    ROM_ADDR_INITIALIZE_YOUR_SANCTUARY_DISPLAY,
    ROM_ADDR_ENABLE_YOUR_SANCTUARY_DISPLAY,
    ROM_ADDR_DISPLAY_YOUR_SANCTUARY_LOCATION,
    ROM_ADDR_INIT_BUBBLE_MONKEY,
    ROM_ADDR_INIT_BUTTERFLY_MOVEMENT,
    ROM_ADDR_UPDATE_BUTTERFLY_MOVEMENT,
    ROM_ADDR_CLEAR_FADE_ENTITY_FLAGS,
    ROM_ADDR_UPDATE_FADE_ENTITY_SPRITES,
    ROM_ADDR_HIDE_FADE_ENTITY_FRAMES,
    ROM_ADDR_ANIMATE_ENTITY_TILE_COPY,
    ROM_ADDR_ANIMATE_ENTITY_TILE_BLEND,
    ROM_ADDR_ANIMATE_ENTITY_TILE_MERGE,
    ROM_ADDR_ADVANCE_TILEMAP_ANIMATION_FRAME,
    ROM_ADDR_DISPLAY_ANIMATION_SEQUENCE_FRAME,
    ROM_ADDR_LOAD_ANIMATION_SEQUENCE_FRAME,
    ROM_ADDR_LOAD_CHARACTER_PORTRAIT_SCREEN,
    ROM_ADDR_UPDATE_SWIRL_EFFECT,
    ROM_ADDR_MOVEMENT_LOAD_BATTLEBG,
    ROM_ADDR_RELOAD_MAP,
    ROM_ADDR_LOAD_MAP_ROW_AT_SCROLL_POS,
    ROM_ADDR_DISPLAY_ANTI_PIRACY_SCREEN,
    ROM_ADDR_LOAD_DEBUG_CURSOR_GRAPHICS,
};

static uint32_t recover_dropped_callroutine_bank(uint32_t rom_addr) {
    if ((rom_addr >> 16) != 0)
        return rom_addr; /* bank byte already present, nothing to recover */
    for (size_t i = 0; i < sizeof(known_callroutine_addrs) / sizeof(known_callroutine_addrs[0]); i++) {
        if ((known_callroutine_addrs[i] & 0xFFFF) == rom_addr)
            return known_callroutine_addrs[i];
    }
...
int16_t callroutine_dispatch(uint32_t rom_addr, int16_t entity_offset,
    ...
    uint32_t recovered = recover_dropped_callroutine_bank(rom_addr);
    if (recovered != rom_addr) {
        LOG_WARN("WARN: callroutine $%06X for entity %d had no bank byte -- "
